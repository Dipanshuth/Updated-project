...
async def run_inference(data: InferenceInput):
    ...
    try:
        ...
        if confidence < 0.20:
            record.ai_summary = (
                f"Audio analysis completed. Distress confidence: {confidence_pct}% (Minimal, ~0-20 dB). "
            ...
            record.ai_summary = (
                f"Audio analysis completed. Distress confidence: {confidence_pct}% (Low, ~20-30 dB). "
                "Quiet speech patterns detected. No significant vocal stress indicators found. "
                "The recording appears to contain conversational audio within normal parameters."
            )
            classification = "Low Distress"
        elif confidence < 0.55:
            record.ai_summary = (
                f"Audio analysis completed. Distress confidence: {confidence_pct}% (Moderate, ~30-40 dB). "
                "Normal conversational vocal energy patterns detected. Some stress indicators present but within "
                "an elevated-but-normal range. Further review may be warranted."
            )
            classification = "Moderate Distress"
        elif confidence < 0.72:
            record.ai_summary = (
                f"Audio analysis completed. Distress confidence: {confidence_pct}% (Medium-High, ~40-50 dB). "
            ...
            classification = "Medium-High Distress"
        elif confidence < 0.85:
            record.ai_summary = (
                f"Audio analysis completed. Distress confidence: {confidence_pct}% (High, ~50-65 dB). "
                "High audio energy detected indicating loud or shouting behavior. "
                "Significant vocal stress markers identified. Likely distress situation."
            )
            classification = "High Distress"
        else:
            record.ai_summary = (
                f"Audio analysis completed. Distress confidence: {confidence_pct}% (Critical, ~65+ dB). "
                "Extreme audio energy detected indicating likely distress or emergency. "
                "Screaming or extreme vocal stress identified. Immediate attention recommended."
            )
