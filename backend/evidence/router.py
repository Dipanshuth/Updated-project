...
def _find_nearest_police_station(lat: float, lng: float) -> dict | None:
    ...
    # Approximate fallback when no station found – provide a generic placeholder based on coordinates
    approx_name = f"Police Station near {lat:.3f}, {lng:.3f}"
    return {"name": approx_name, "distance_km": None, "address": ""}
