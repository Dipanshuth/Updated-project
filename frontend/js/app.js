...
function populateEvidenceTable(data) {
  ...
  data.forEach(item => {
    ...
    const confidenceDisplay = item.confidence !== null && item.confidence !== undefined
      ? `<span style="font-weight:700;color:${item.confidence > 72 ? 'var(--danger)' : item.confidence > 55 ? 'var(--warning)' : 'var(--success)'};">${item.confidence}%</span>`
...
function populateActivityFeed(data) {
  ...
  recent.forEach((item, idx) => {
    ...
    if (item.status === 'analyzed' && item.confidence !== null) {
      ...
      text = `Evidence #${item.id} analyzed — ${item.confidence > 72 ? 'Distress detected' : 'No distress'} (${item.confidence}% confidence)`;
...
function updateStats(data) {
  ...
  if (firEl) firEl.textContent = data.filter(i => i.status === 'analyzed' && i.confidence > 72).length;
...
async function loadIncidentDetail(evidenceId) {
  try {
    ...
    if (metaDistress) {
      ...
      const isDistress = data.distress_detected || pct >= 72;
      const level = data.distress_level || 'Unknown';

      if (isDistress) {
        metaDistress.textContent = `${level} (${pct}%)`;
        metaDistress.style.color = pct >= 72 ? 'var(--danger)' : pct >= 55 ? 'var(--warning)' : 'var(--success)';
      } else {
        metaDistress.textContent = 'Normal Sound';
    ...
    if (confPctEl) {
      ...
      if (pct >= 72) confPctEl.style.color = 'var(--danger)';
      else if (pct >= 55) confPctEl.style.color = 'var(--warning)';
    ...
    if (audioEl && data.file_path) {
      const pathParts = data.file_path.split(/[/\\\\]/).filter(Boolean);
      const fileName = pathParts.length ? pathParts[pathParts.length - 1] : data.file_path;
      const audioSrc = `${API_BASE}/uploads/${encodeURIComponent(fileName)}`;
...
async function analyzeWithAI(evidenceId) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      const isDistress = result.distress_detected || pct >= 72;
      const level = result.distress_level || 'Unknown';
      const message = isDistress
        ? `Analysis complete — Distress: ${level} (${pct}%)`
        : `Analysis complete — Normal sound detected (${pct}%)`;
